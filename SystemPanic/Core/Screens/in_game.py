...
def advance_in_game(paks, game_state, time_since_start, delta_t):
    from SystemPanic.Core.game_state import (
        next_level,
        change_mode,
        GAME_MODES,
        new_missile,
    ...
    game_state["enemy_missiles"] = [
        missile for missile in game_state["enemy_missiles"] if missile["active"] is True
    ]
    ...
    # Prune dead enemies
    game_state["enemies"] = [
        enemy for enemy in game_state["enemies"] if enemy["active"] is True
    ]
...
def check_player_to_enemy_collisions(game_state):
    for enemy in game_state["enemies"]:
        if do_sprites_collide(game_state["players"][0], enemy):
            game_state["active_config"]["players"]["collided_with_enemy"](
                game_state["players"][0], enemy
            )
            game_state["active_config"]["enemies"]["collided_with_player"](
                enemy, game_state["players"][0]
            )
...
def check_player_to_enemy_missile_collisions(game_state):
    for missile in game_state["enemy_missiles"]:
        if do_sprites_collide(game_state["players"][0], missile):
            game_state["active_config"]["players"]["collided_with_enemy_missile"](
                game_state["players"][0], missile
            )
            game_state["active_config"]["enemy_missiles"]["collided_with_player"](
                missile, game_state["players"][0]
            )
        ...
        for missile in game_state["player_missiles"]:
            if enemy["active"] is True and missile["active"] is True:
                if do_sprites_collide(enemy, missile):
                    game_state["score"] += 1
                    game_state["active_config"]["enemies"][
                        "collided_with_player_missile"
                    ](enemy, missile)
                    game_state["active_config"]["player_missiles"][
                        "collided_with_enemy"
                    ](missile, enemy)
    ...
    for wall in game_state["walls"]:
        if wall["active"] is True:
            ...
            if do_sprites_collide(wall, game_state["players"][0]):
                game_state["players"][0] = game_state["active_config"]["players"][
                    "collided_with_level"
                ](
            ...
            for missile in game_state["player_missiles"]:
                if do_sprites_collide(wall, missile):
                    game_state["active_config"]["player_missiles"][
                        "collided_with_level"
                    ](missile, missile["previous_position"])

            # Enemy Missiles
            for missile in game_state["enemy_missiles"]:
                if do_sprites_collide(wall, missile):
                    game_state["active_config"]["enemy_missiles"][
                        "collided_with_level"
                    ](missile, missile["previous_position"])
