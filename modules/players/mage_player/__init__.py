...
class Player(BasePlayer):
    SPRITESHEET_LAYOUT_NOTE = (
        "Frames (32x48px): Idle, Walk1, Walk2, CastStart, Casting. Horizontal."
    )
    SPRITE_WIDTH = 32
    SPRITE_HEIGHT = 48
    ...
    def load_assets(module_path):
        ...
        try:
            ...
            if os.path.exists(projectile_image_path):
                ...
                proj_img = pygame.Surface((12, 12), pygame.SRCALPHA)
                proj_img.fill((0, 150, 255))
    ...
    def update(self, keys, platforms, dt):
        ...
        if not self.is_casting_anim:
            if keys[pygame.K_LEFT]:
                ...
                if self.on_ground and self.anim_state == "walk":
        ...
        if (
            not self.can_attack
            and current_time_ms - self.last_attack_timestamp > self.attack_cooldown_time
        ):
            self.can_attack = True

        if (
            self.is_casting_anim
            and current_time_ms - self.last_attack_timestamp > self.cast_anim_duration
        ):
        ...
        if self.rect.top > 600 + Player.SPRITE_HEIGHT:
            self.take_damage(self.health)
        self.rect.clamp_ip(
            pygame.Rect(
                0, -Player.SPRITE_HEIGHT * 2, 800, 600 + Player.SPRITE_HEIGHT * 2
            )
        )
    ...
    def collide_with_platforms(
        self, dx, dy, platforms
    ):  # Identical to Knight's, could be base class
    ...
    def set_anim_state(self, new_state):
        if self.anim_state != new_state or (
            new_state == "attack" and not self.is_casting_anim
        ):  # Allow re-triggering attack anim
            self.anim_state = new_state
            self.current_frame_idx = 0
            self.last_anim_update = pygame.time.get_ticks()
            if (
                new_state == "attack"
            ):  # Mage specific: 'attack' state refers to casting animation
    ...
    def animate(self, current_time_ms):
        ...
        if self.anim_state == "idle":
            ...
            idx = 0  # Mage uses idle for jump, or add specific jump frame
            ...
            self.image = pygame.transform.flip(
                current_image, self.facing_direction == -1, False
            )
            return
    ...
    def attack(self):
        ...
        self.projectiles.add(
            MageProjectile(proj_x, proj_y, self.facing_direction, self.assets)
        )
