...
class Player(BasePlayer):
    ...
    def update(self, keys, platforms, dt):
        ...
        if keys[pygame.K_LEFT]:
            ...
            if self.on_ground and self.anim_state != "attack":
        ...
        if self.vel_y > 15:
            self.vel_y = 15  # Terminal velocity
        dy = (
            self.vel_y
        )  # For this frame, vel_y already includes gravity effect over time if dt was used for gravity too
        ...
        self.rect.y += dy
        self.on_ground = (
            False  # Assume not on ground until proven by vertical collision
        )
        self.collide_with_platforms(0, dy, platforms)

        if self.rect.top > 600 + Player.SPRITE_HEIGHT:
            self.take_damage(self.health)  # Fell off
        self.rect.clamp_ip(
            pygame.Rect(
                0, -Player.SPRITE_HEIGHT * 2, 800, 600 + Player.SPRITE_HEIGHT * 2
            )
        )  # Clamp to screen with some leeway for jumping up
    ...
    def attack(self):
        ...
        self.projectiles.add(
            Projectile(proj_x, proj_y, self.facing_direction, self.assets)
        )
