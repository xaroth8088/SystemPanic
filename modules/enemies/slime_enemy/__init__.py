...
class Enemy(BaseEnemy):
    ...
    def load_assets(module_path):
        ...
        try:
            ...
            for i in range(sheet.get_width() // Enemy.SPRITE_WIDTH):
                frame = sheet.subsurface(pygame.Rect(i * Enemy.SPRITE_WIDTH, 0, Enemy.SPRITE_WIDTH, Enemy.SPRITE_HEIGHT))
                assets['frames'].append(frame)
        except pygame.error as e:
            print(f"Error loading Slime enemy assets: {e}")
            assets['frames'] = [pygame.Surface((Enemy.SPRITE_WIDTH,Enemy.SPRITE_HEIGHT), pygame.SRCALPHA) for _ in range(2)]
    ...
    def __init__(self, x, y, assets, player_rect_for_ai): # player_rect can be None
        ...
        self.image = self.frames[0] if self.frames else pygame.Surface([Enemy.SPRITE_WIDTH, Enemy.SPRITE_HEIGHT]); self.image.fill((0,180,0))
    ...
    def update(self, platforms, player_rect, dt): 
        ...
        if player_rect:
            ...
            if self.ai_state == "patrol":
                if abs(dist_x) < self.chase_dist_x and dist_y < self.chase_dist_y:
                    ...
                    if current_time_ms - self.last_patrol_dir_change > self.patrol_change_dir_interval:
                ...
                if abs(dist_x) > self.lose_aggro_dist_x or dist_y > self.chase_dist_y * 2:
                    self.ai_state = "patrol"
            ...
            self.ai_state = "patrol"
            if current_time_ms - self.last_patrol_dir_change > self.patrol_change_dir_interval:
                self.facing_direction *= -1; self.last_patrol_dir_change = current_time_ms
            dx = speed * self.facing_direction
        ...
        if self.rect.top > 600 + Enemy.SPRITE_HEIGHT : self.kill() 
    ...
    def collide_with_platforms(self, dx, dy, platforms):
        for platform in platforms:
            if self.rect.colliderect(platform.rect):
                if dx > 0: self.rect.right = platform.rect.left; self.facing_direction = -1 # Turn on wall hit
                if dx < 0: self.rect.left = platform.rect.right; self.facing_direction = 1  # Turn on wall hit
                if dy > 0: self.rect.bottom = platform.rect.top; self.on_ground = True; self.vel_y = 0
    ...
    def animate(self, current_time_ms):
        ...
        # Slimes usually don't flip, but if sprite is directional:
        # self.image = pygame.transform.flip(new_image, self.facing_direction == -1, False)
        self.image = new_image

    def take_damage(self, amount):
        self.health -= amount
        if self.health <= 0: self.health = 0 # is_alive will handle kill

    def is_alive(self): return self.health > 0
    def deal_damage_on_contact(self): return 10
