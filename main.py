...
def load_module_from_path(module_name, path_to_module_file):
    ...
    if spec is None:
        print(f"Error: Could not load spec for module {module_name} at {path_to_module_file}")
        return None
...
def list_module_dirs(base_module_type_path):
    ...
    return [
        ...
        if os.path.isdir(os.path.join(base_module_type_path, d)) and not d.startswith("__")
    ]
...
def load_assets_for_module_class(module_class, module_path_dir):
    ...
    if hasattr(module_class, 'load_assets'):
        try:
            ...
            print(f"Error loading assets for {module_class.__name__} in {module_path_dir}: {e}")
            return {} 
    return {}
...
class Game:
    def __init__(self):
        ...
        self.player_module_dirs = list_module_dirs(os.path.join(os.getcwd(), "modules", "players"))
        self.enemy_module_dirs = list_module_dirs(os.path.join(os.getcwd(), "modules", "enemies"))
        self.platform_module_dirs = list_module_dirs(os.path.join(os.getcwd(), "modules", "platforms"))

        if not self.player_module_dirs or not self.enemy_module_dirs or not self.platform_module_dirs:
            print("Error: Missing one or more module types (players, enemies, platforms).")
        ...
        self.all_projectiles = pygame.sprite.Group() # Central group for all projectiles
    ...
    def randomize_modules(self, initial_setup=False):
        ...
        if not self.player_module_dirs: print("No player modules found!"); self.running = False; return
        new_player_module_path_dir = random.choice(self.player_module_dirs)

        if not initial_setup and self.active_player_module_path == new_player_module_path_dir and self.player:
            print(f"Player module '{os.path.basename(new_player_module_path_dir)}' unchanged. Keeping existing player.")
            # Ensure PlayerClass is still valid if it was somehow cleared
            if not self.PlayerClass:  # Should not happen if player instance exists and path is same
            ...
            print(f"Switching to player module: {os.path.basename(new_player_module_path_dir)}")
            player_module_file = os.path.join(new_player_module_path_dir, "__init__.py")
            player_module = load_module_from_path("player_module", player_module_file)

            if not player_module or not hasattr(player_module, 'Player'):
                print(f"Failed to load Player from {new_player_module_path_dir}. Keeping previous or exiting.")
                if initial_setup and not self.PlayerClass: self.running = False; return
                # If not initial setup and load fails, the old player (if any) and PlayerClass remain.
            else:
                new_PlayerClass = player_module.Player
                player_assets = load_assets_for_module_class(new_PlayerClass, new_player_module_path_dir)
                ...
                if self.player:
                    ...
                    if hasattr(self.player, 'health'): old_health = self.player.health
                ...
                if hasattr(self.player, 'health'): self.player.health = old_health
                ...
                if hasattr(self.player, 'projectiles'):  # New player has its own projectile group
                    self.all_projectiles.add(self.player.projectiles)

                self.active_player_module_path = new_player_module_path_dir

        # --- Select Enemy module ---
        if not self.enemy_module_dirs: print("No enemy modules found!"); self.running = False; return
        new_enemy_module_path_dir = random.choice(self.enemy_module_dirs)

        if not initial_setup and self.active_enemy_module_path == new_enemy_module_path_dir and self.EnemyClass:
            print(f"Enemy module '{os.path.basename(new_enemy_module_path_dir)}' unchanged. Existing enemies retained.")
            ...
            print(f"Switching to enemy module: {os.path.basename(new_enemy_module_path_dir)}")
            ...
            if not enemy_module or not hasattr(enemy_module, 'Enemy'):
                print(f"Failed to load Enemy from {new_enemy_module_path_dir}. Keeping previous or exiting.")
                if initial_setup and not self.EnemyClass: self.running = False; return
            else:
                new_EnemyClass = enemy_module.Enemy
                enemy_assets = load_assets_for_module_class(new_EnemyClass, new_enemy_module_path_dir)

                existing_enemies_data = []
                for enemy in self.enemies:
                    data = {'pos': enemy.rect.topleft}
                    if hasattr(enemy, 'health'): data['health'] = enemy.health
                    # Add other generic enemy states if needed (e.g., velocity, facing_direction)
                    for attr in ['vel_x', 'vel_y', 'facing_direction', 'ai_state']:  # ai_state might be too specific
                        if hasattr(enemy, attr): data[attr] = getattr(enemy, attr)
                ...
                for data in existing_enemies_data:
                    ...
                    new_enemy_instance = self.EnemyClass(data['pos'][0], data['pos'][1], enemy_assets,
                                                         player_rect_for_ai)

                    if 'health' in data and hasattr(new_enemy_instance, 'health'):
                        new_enemy_instance.health = data['health']
                    for attr, value in data.items():
                        if attr not in ['pos', 'health'] and hasattr(new_enemy_instance, attr):
        ...
        if not self.platform_module_dirs: print("No platform modules found!"); self.running = False; return
        new_platform_module_path_dir = random.choice(self.platform_module_dirs)

        if not initial_setup and self.active_platform_module_path == new_platform_module_path_dir and self.platform_creation_func:
            print(
            ...
            if self.platform_module:  # Assuming self.platform_module is the loaded module object
                load_assets_for_platform_module(self.platform_module, new_platform_module_path_dir)

        else:
            print(f"Switching to platform module: {os.path.basename(new_platform_module_path_dir)}")
            platform_module_file = os.path.join(new_platform_module_path_dir, "__init__.py")
            loaded_platform_module = load_module_from_path("platform_module", platform_module_file)  # Use a temp var

            if not loaded_platform_module or not hasattr(loaded_platform_module, 'create_platforms') or not hasattr(
                    loaded_platform_module, 'load_platform_assets'):
                print(f"Failed to load Platform module from {new_platform_module_path_dir} or it's malformed.")
                if initial_setup and not self.platform_creation_func: self.running = False; return
            else:
                self.platform_module = loaded_platform_module  # Store the loaded module itself
                self.platform_creation_func = self.platform_module.create_platforms
                self.platform_module_assets = load_assets_for_platform_module(self.platform_module,
                                                                              new_platform_module_path_dir)

                for p in self.platforms: p.kill()  # Kill old platforms
                self.platforms.empty()

                new_platform_sprites = self.platform_creation_func(SCREEN_WIDTH, SCREEN_HEIGHT,
                                                                   self.platform_module_assets)
    ...
    def spawn_enemies(self):
        ...
        if not enemy_assets:
            print(f"Warning: Assets for active enemy module {self.active_enemy_module_path} not found in cache during spawn. Attempting load.")
            ...
            if self.EnemyClass: # Check if EnemyClass is available
                 enemy_assets = load_assets_for_module_class(self.EnemyClass, self.active_enemy_module_path)
            if not enemy_assets:
                 print(f"Critical: Failed to get/load assets for {self.active_enemy_module_path}. Cannot spawn enemies.")
                 return
        ...
        for _ in range(num_to_spawn_now):
            ...
            while spawn_attempts < 100: # Max attempts to prevent infinite loop
                ...
                y = random.randint(0, SCREEN_HEIGHT // 2) # Spawn in upper half generally

                distance_ok = True
                if self.player:
                    distance = math.hypot(x - self.player.rect.centerx, y - self.player.rect.centery)
                ...
                enemy_width_approx = self.EnemyClass.SPRITE_WIDTH if hasattr(self.EnemyClass, 'SPRITE_WIDTH') else 32
                enemy_height_approx = self.EnemyClass.SPRITE_HEIGHT if hasattr(self.EnemyClass, 'SPRITE_HEIGHT') else 32
                ...
                if distance_ok and not collision_with_platform:
                    new_enemy = self.EnemyClass(x, y, enemy_assets, self.player.rect) # Pass player_rect for AI
                    self.enemies.add(new_enemy)
                    break 
                spawn_attempts += 1
            else: # while loop finished due to max_attempts
                print(f"Warning: Could not find suitable spawn location for an enemy after {spawn_attempts} attempts.")

        print(f"Spawned {num_to_spawn_now} new enemies. Total: {len(self.enemies)}. Target: {self.enemies_to_spawn_count}")
    ...
    def run(self):
        while self.running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if self.game_over and event.key == pygame.K_r:
                        self.restart_game()
                    if not self.game_over and self.player and hasattr(self.player, 'handle_event'):
                        self.player.handle_event(event) # For single key presses if player module uses it
            ...
            if current_time - self.last_module_randomize_time > MODULE_RANDOMIZE_INTERVAL:
                self.randomize_modules()

            # --- Updates ---
            keys = pygame.key.get_pressed()
            if self.player:
                self.player.update(keys, self.platforms, dt) # Pass dt for frame-independent movement/physics
            ...
            if self.player:
                ...
                for enemy_hit in enemy_hit_list:
                    if hasattr(self.player, 'take_damage') and hasattr(enemy_hit, 'deal_damage_on_contact'):
                ...
                for proj in self.all_projectiles:
                    ...
                    for enemy_hit in hit_enemies:
                        if hasattr(enemy_hit, 'take_damage') and hasattr(proj, 'damage'):
            ...
            if not self.enemies and not self.game_over and self.player: # All enemies defeated this wave
                print("Wave cleared!")
                self.enemies_to_spawn_count += 1 
                self.randomize_modules() # Randomize for next wave
                if self.running and self.player: # Ensure game still running and player exists
                     self.spawn_enemies()
            ...
            if self.player and hasattr(self.player, 'health'):
                ...
                health_text = self.font.render(f"Health: {health_val}", True, GREEN if health_val > 30 else RED)
                self.screen.blit(health_text, (SCREEN_WIDTH - 150, 10))

            enemies_left_text = self.font.render(f"Enemies: {len(self.enemies)}/{self.enemies_to_spawn_count}", True, WHITE)
            self.screen.blit(enemies_left_text, (10, 40))

            time_to_random = (MODULE_RANDOMIZE_INTERVAL - (current_time - self.last_module_randomize_time)) // 1000
            random_timer_text = self.font.render(f"Shuffle in: {max(0, time_to_random)}s", True, WHITE)
            self.screen.blit(random_timer_text, (SCREEN_WIDTH // 2 - random_timer_text.get_width()//2, 10))
    ...
    def draw_game_over(self):
        ...
        self.screen.blit(restart_text, (SCREEN_WIDTH // 2 - restart_text.get_width() // 2, SCREEN_HEIGHT * 2 // 3))
    ...
    def restart_game(self):
        ...
        if self.player: self.player.kill(); self.player = None
